...
const PORTFOLIO_MANAGER_OPEN_STORAGE_KEY =
  "complete-portfolio-report-manager-open-v1";
...
function getPortfolioManagerKey(asset: Asset): string {
  // Presentation settings deliberately use the original Overview name.
  // Entity/status are NOT part of the key, so settings remain available
  // when Overview later moves the same property from Current to Sold.
  const originalName = asset.sourceProject ?? asset.project ?? asset.address;

  return [asset.country, originalName, asset.purchaseDate]
    .map(normalizeText)
    .join("|");
}
...
function getPortfolioAssetSetting(
  ...
  const stored = settings[getPortfolioManagerKey(asset)] ?? {};

  return {
    reportName: stored.reportName ?? asset.project,
    category: stored.category ?? getSoldCategory(asset),
  };
}
...
function applyPortfolioAssetSetting(
  ...
  const key = getPortfolioManagerKey(asset);
  ...
  return {
    ...
    customPhotoUrl: photoUrls[key] ?? "",
  };
}

function getPortfolioReportCategory(asset: Asset): SoldCategoryId {
  return asset.reportCategory ?? getSoldCategory(asset);
}
...
async function loadPortfolioPhoto(assetKey: string): Promise<Blob | null> {
  ...
  const result = await new Promise<Blob | null>((resolve, reject) => {
    ...
    const request = transaction
      .objectStore(PORTFOLIO_PHOTO_STORE_NAME)
      .get(assetKey);
...
export default function CompletePortfolioPage() {
  ...
  const [portfolioAssetSettingsReady, setPortfolioAssetSettingsReady] =
    useState(false);
  const [portfolioPhotoUrls, setPortfolioPhotoUrls] =
  ...
  const updatePortfolioAssetSetting = (
    ...
    const assetKey = getPortfolioManagerKey(asset);
  ...
  const updatePortfolioAssetPhoto = async (
    ...
    setPortfolioPhotoUrls((current) => {
      ...
      return { ...current, [assetKey]: nextUrl };
    });
  };

  const deletePortfolioAssetPhoto = async (asset: Asset) => {
    const assetKey = getPortfolioManagerKey(asset);
    await removePortfolioPhoto(assetKey);
    ...
    try {
      ...
      const validSettings = Object.fromEntries(
        Object.entries(parsedValue).flatMap(([key, value]) => {
          ...
          if (typeof candidate.reportName === "string") {
            setting.reportName = candidate.reportName;
          }

          if (isSoldCategoryId(candidate.category)) {
            setting.category = candidate.category;
          }
  ...
  useEffect(() => {
    ...
    async function hydratePortfolioPhotos() {
      const visibleSourceAssets = data.assets.filter(
        (asset) => isCurrentAsset(asset) || isSoldAsset(asset),
      );

      const entries = await Promise.all(
        visibleSourceAssets.map(async (asset) => {
          const assetKey = getPortfolioManagerKey(asset);

          try {
            const blob = await loadPortfolioPhoto(assetKey);
            return blob ? ([assetKey, URL.createObjectURL(blob)] as const) : null;
          } catch {
            return null;
          }
        }),
      ...
      if (cancelled) {
        entries.forEach((entry) => {
          if (entry?.[1]?.startsWith("blob:")) URL.revokeObjectURL(entry[1]);
        });
        return;
      }
    ...
    if (data.assets.length > 0) void hydratePortfolioPhotos();
  ...
  const portfolioManagerAssets = useMemo(
    () =>
      data.assets.filter(
        (asset) => isCurrentAsset(asset) || isSoldAsset(asset),
      ),
  ...
  const reportSharedAssets = metrics.sharedAssets.map((asset) =>
  ...
  const reportWhollyOwnedAssets = metrics.whollyOwnedAssets.map((asset) =>
    applyPortfolioAssetSetting(
      asset,
      portfolioAssetSettings,
      portfolioPhotoUrls,
  ...
  const reportSoldAssets = metrics.soldAssets.map((asset) =>
    applyPortfolioAssetSetting(
      asset,
      portfolioAssetSettings,
      portfolioPhotoUrls,
  ...
  const soldCategoryPages = SOLD_CATEGORY_DEFINITIONS.flatMap(
    (definition) => {
      const assets = sortSoldAssetsByAddress(
        reportSoldAssets.filter(
          (asset) =>
            asset.country !== "Spanje" &&
            getPortfolioReportCategory(asset) === definition.id,
        ),
      );
  ...
  const spanishSoldPages = chunk(
    reportSoldAssets.filter((asset) => asset.country === "Spanje"),
    2,
  );

  const trackRecordStartNumber =
    whollyOwnedStartNumber + reportWhollyOwnedAssets.length;
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fa] py-8 print:bg-white print:py-0">
      ...
      {metrics.sharedAssets.length > 0 && (
        <PageFrame>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            {chunk<Asset>(
              reportSharedAssets,
              Math.ceil(reportSharedAssets.length / 2),
      ...
      {reportWhollyOwnedAssets.map((asset, index) => (
        <PageFrame key={`${asset.entity}-${asset.project}`}>
          ...
          <div className="mt-5 grid grid-cols-[1.15fr_1fr] gap-5">
            <ProjectImage
              ...
              customPhotoUrl={asset.customPhotoUrl}
              className="h-[114mm]"
            />
...
function PortfolioManager({
  ...
  useEffect(() => {
    try {
      const stored = window.localStorage.getItem(
        PORTFOLIO_MANAGER_OPEN_STORAGE_KEY,
      );
  ...
  useEffect(() => {
    try {
      ...
      // De open/dicht-status is alleen een UI-voorkeur.
    }
  }, [isOpen]);
  ...
  const currentCount = assets.filter(isCurrentAsset).length;
  const soldCount = assets.filter(isSoldAsset).length;
  ...
  return (
    <section className="no-print mx-auto mb-8 w-[min(1220px,calc(100%-32px))] border border-[#cbd5e1] bg-[#f5f7fa] shadow-xl">
      <button
        ...
        className="flex w-full items-center justify-between gap-8 bg-[#0f172a] px-6 py-4 text-left text-white"
        aria-expanded={isOpen}
      >
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-[0.28em] text-[#94a3b8]">
            Live invoersheet
          </p>
          <div className="mt-1 flex items-center gap-3">
            <h2 className="text-xl font-semibold">Portfolio Manager</h2>
        ...
        <div className="flex items-center gap-4">
          <div className="grid grid-cols-3 border border-white/15 text-center">
            <div className="min-w-[78px] px-3 py-2">
              <p className="text-[7px] uppercase tracking-[0.17em] text-white/45">
                Total
              </p>
              <p className="mt-0.5 text-base font-semibold">{assets.length}</p>
            </div>
            <div className="min-w-[78px] border-l border-white/15 px-3 py-2">
              <p className="text-[7px] uppercase tracking-[0.17em] text-white/45">
                Current
              </p>
              <p className="mt-0.5 text-base font-semibold">{currentCount}</p>
            </div>
            <div className="min-w-[78px] border-l border-white/15 px-3 py-2">
              <p className="text-[7px] uppercase tracking-[0.17em] text-white/45">
                Sold
              </p>
              <p className="mt-0.5 text-base font-semibold">{soldCount}</p>
            </div>
          </div>

          <span className="flex h-8 w-8 items-center justify-center border border-white/20 text-lg font-medium">
            {isOpen ? "−" : "+"}
          </span>
        </div>
      </button>
      ...
      {isOpen && (
        <>
          <div className="border-b border-[#cbd5e1] px-6 py-4">
            <p className="max-w-4xl text-[11px] leading-5 text-[#475569]">
              De eerste kolom toont de naam rechtstreeks uit Overview. Alleen
              rapportnaam, categorie en foto worden hier opgeslagen. Current en
              Sold zijn alleen-lezen en worden uitsluitend door de live
              Overview-data bepaald.
            </p>
          </div>
          ...
          <div className="flex items-center justify-between border-b border-[#cbd5e1] px-6 py-4">
            <div>
              <p className="text-[9px] font-semibold uppercase tracking-[0.18em] text-[#64748b]">
                Alle huidige en verkochte panden
              </p>
              <p className="mt-1 text-[10px] text-[#64748b]">
                {visibleAssets.length} van {assets.length} zichtbaar
              </p>
            </div>
            ...
            <input
              ...
              className="w-[390px] border border-[#cbd5e1] bg-white px-4 py-2.5 text-[11px] text-[#0f172a] outline-none placeholder:text-[#94a3b8] focus:border-[#64748b]"
            />
          </div>

          <div className="mx-6 mb-6 mt-4 max-h-[590px] overflow-auto border border-[#cbd5e1] bg-white">
            <div className="sticky top-0 z-20 grid grid-cols-[1.35fr_1.35fr_.9fr_1.08fr_.62fr] gap-3 border-b border-[#cbd5e1] bg-[#eef2f6] px-4 py-3 text-[8px] font-semibold uppercase tracking-[0.15em] text-[#475569]">
            ...
            {visibleAssets.map((asset, index) => {
              ...
              const status = isSoldAsset(asset) ? "Sold" : "Current";

              const handleFile = (file: File | undefined) => {
                if (file) void onPhotoChange(asset, file);
              };

              return (
                <div
                  key={`${assetKey}-${index}`}
                  className={`grid min-h-[76px] grid-cols-[1.35fr_1.35fr_.9fr_1.08fr_.62fr] items-center gap-3 px-4 py-2.5 text-[10px] ${
                ...
                >
                  ...
                  <input
                    ...
                    className="min-w-0 border border-[#cbd5e1] bg-white px-3 py-2 text-[10px] font-medium text-[#0f172a] outline-none focus:border-[#64748b]"
                  />
                  ...
                  <select
                    ...
                    className="w-full border border-[#cbd5e1] bg-white px-3 py-2 text-[10px] font-semibold text-[#0f172a] outline-none focus:border-[#64748b]"
                  >
                  ...
                  <div
                    ...
                    className="grid min-h-[52px] grid-cols-[58px_1fr] overflow-hidden border border-[#cbd5e1] bg-white"
                  >
                    <div className="flex items-center justify-center border-r border-[#cbd5e1] bg-[#eef2f6]">
                      {previewPhoto ? (
                        <img
                          src={previewPhoto}
                          alt=""
                        ...
                        <span className="text-[7px] font-semibold uppercase tracking-[0.12em] text-[#94a3b8]">
                          Foto
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex min-w-0 items-center justify-between gap-2 px-2">
                      <label className="cursor-pointer text-[8px] font-semibold text-[#475569]">
                        Sleep of kies foto
                        <input
                          type="file"
                          accept="image/*"
                      ...
                      {uploadedPhoto && (
                        <button
                          ...
                          className="shrink-0 text-[8px] font-semibold text-[#b42318]"
                        >
                          ×
                        </button>
                      )}
                  ...
                  <span className="w-fit border border-[#cbd5e1] bg-[#f8fafc] px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#0f172a]">
                    {status}
                  </span>
                </div>
              );
            ...
            {visibleAssets.length === 0 && (
              <p className="px-5 py-10 text-center text-[11px] text-[#64748b]">
                Geen panden gevonden voor deze zoekopdracht.
              </p>
            )}
          </div>
        </>

...
const SOLD_CATEGORY_DEFINITIONS: SoldCategoryDefinition[] = [
  ...
  },
  {
    ...
    subtitle: "Realized industrial portfolio",
  },
];

// Handmatige indeling van de huidige Nederlandse verkochte objecten.
// Expliciete woorden als “office”, “residential” of “industrial” in de
// projectnaam blijven altijd leidend.
...
const SOLD_FEATURED_PROJECT_RULES: Record<SoldCategoryId, RegExp[]> = {
  ...
  residential: [
    /prinsessesingel 13|princessesingel 13/,
    /groethofstraat 103|goethofstraat 103/,
  ],
  industrial: [
    /rudolf dieselweg 34/,
    /transportlaan 1/,
...
function getSoldCategory(asset: Asset): SoldCategoryId {
  ...
  // Industrial Halls, zodat geen verkocht object uit het overzicht verdwijnt.
  return "industrial";
}
...
function getEffectiveSoldCategory(
  ...
  return overrides[getSoldAssetKey(asset)] ?? getSoldCategory(asset);
}
...
function getFeaturedSoldAssets(
  ...
  for (const rule of preferredRules) {
    const preferredAsset = assets.find((asset) => {
      ...
      return (
        rule.test(text) &&
        Boolean(getImageSource(asset.project, asset.address))
      );
    });
...
export default function CompletePortfolioPage() {
  ...
  const soldCategoryPages = SOLD_CATEGORY_DEFINITIONS.map((definition) => ({
    ...definition,
    assets: sortSoldAssetsByAddress(
...
function SoldCategoryEditor({
  ...
  const sortedAssets = useMemo(
    () =>
      [...soldAssets].sort((left, right) => {
        ...
        return getSoldDisplayAddress(left).localeCompare(
          getSoldDisplayAddress(right),
          "nl",
          { numeric: true, sensitivity: "base" },
        );
  ...
  const visibleAssets = normalizedSearch
    ? sortedAssets.filter((asset) =>
        normalizeText(
          `${asset.project} ${asset.address} ${asset.entity} ${asset.country}`,
  ...
  const manualCount = soldAssets.filter(
    (asset) => overrides[getSoldAssetKey(asset)] !== undefined,
  ).length;

  return (
    <section className="no-print mx-auto mb-8 w-[min(1180px,calc(100%-32px))] overflow-hidden rounded-[28px] border border-[#d8d0c1] bg-[#f7f4ec] shadow-xl">
      <div className="flex items-start justify-between gap-6 bg-[#20382f] px-6 py-5 text-white">
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-[0.3em] text-[#d9b17a]">
            Live invoersheet
          </p>
          ...
          <p className="mt-2 max-w-2xl text-[11px] leading-5 text-white/65">
            Kies per Nederlands verkocht pand de juiste categorie. De
            trackrecordpagina&apos;s worden direct bijgewerkt en de keuzes
            blijven op deze browser bewaard. Spaanse objecten blijven op de
            afzonderlijke detailpagina&apos;s.
          </p>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          >
            Herstel automatische indeling
          </button>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-3 border-b border-[#d8d0c1] px-6 py-4">
      ...
      <div className="flex items-center justify-between gap-4 px-6 py-4">
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-[0.18em] text-[#9a6f37]">
            Verkochte objecten
          </p>
          <p className="mt-1 text-[10px] text-[#6f7974]">
        ...
        <input
          ...
          className="w-[340px] rounded-full border border-[#d1c7b8] bg-white px-4 py-2.5 text-[11px] text-[#243d33] outline-none placeholder:text-[#9a9f9c] focus:border-[#9a6f37]"
        />
      </div>

      <div className="mx-6 mb-6 max-h-[520px] overflow-auto rounded-[18px] border border-[#d8d0c1] bg-white">
        <div className="sticky top-0 z-10 grid grid-cols-[1.2fr_1.7fr_.55fr_.65fr_1fr_.55fr] gap-3 bg-[#eee8dd] px-4 py-3 text-[8px] font-semibold uppercase tracking-[0.15em] text-[#6f6256]">
          <span>Project</span>
          <span>Adres</span>
          <span>Land</span>
        ...
        {visibleAssets.map((asset, index) => {
          ...
          const assetKey = getSoldAssetKey(asset);
          const hasManualCategory = overrides[assetKey] !== undefined;
            ...
            <div
              ...
              className={`grid min-h-[48px] grid-cols-[1.2fr_1.7fr_.55fr_.65fr_1fr_.55fr] items-center gap-3 px-4 py-2.5 text-[10px] ${
                index !== visibleAssets.length - 1
                  ? "border-b border-[#e5dfd5]"
                  : ""
              }`}
            >
              <p className="truncate font-semibold text-[#243d33]">
                {asset.project || "—"}
              </p>
              <p className="truncate text-[#65706b]">
                {getSoldDisplayAddress(asset)}
              </p>
              <p className="text-[#65706b]">
                {asset.country}
              </p>
              <p className="whitespace-nowrap text-right font-semibold text-[#243d33]">
                {area.value > 0
              ...
              {isSpanishAsset ? (
                <div className="rounded-[10px] border border-[#d7cbb9] bg-[#f4efe6] px-3 py-2 text-[9px] font-semibold text-[#8b6840]">
                  Spanje · detailpagina
                </div>
              ) : (
                <select
                  ...
                  className="w-full rounded-[10px] border border-[#d7cbb9] bg-white px-3 py-2 text-[10px] font-semibold text-[#243d33] outline-none focus:border-[#9a6f37]"
                >
              ...
              <span
                className={`w-fit rounded-full px-2.5 py-1 text-[8px] font-semibold ${
                  isSpanishAsset
                    ? "bg-[#e8eee9] text-[#597066]"
                    : hasManualCategory
                      ? "bg-[#e7dccb] text-[#8b6840]"
                      : "bg-[#edf0ec] text-[#68736d]"
                }`}
              >
                {isSpanishAsset
                  ? "Vast"
        ...
        {visibleAssets.length === 0 && (
          <p className="px-5 py-10 text-center text-[11px] text-[#747d78]">
...
function SoldCategoryOverviewPage({
  ...
  number: string;
  category: SoldCategoryDefinition & { assets: Asset[] };
  updated: Date | null;
}) {
  ...
  return (
    <PageFrame>
      <ReportHeader
        ...
        subtitle={`${category.subtitle} · ${category.assets.length} realized assets`}
      />
      ...
      <div className="mt-5 grid grid-cols-[0.82fr_1.38fr] gap-5">
        ...
        <section className="overflow-hidden rounded-[22px] border border-[#d8d0c1] bg-white/85">
          ...
          <div className="grid grid-cols-2 divide-x divide-[#ded7ca]">
...
function FeaturedSoldAsset({ asset }: { asset: Asset }) {
  ...
  return (
    <article className="overflow-hidden rounded-[20px] border border-[#d8d0c1] bg-white/85">
      <ProjectImage
        project={asset.project}
        address={asset.address}
      ...
      <div className="flex items-end justify-between gap-4 px-4 py-3">
        <div className="min-w-0">
          <p className="text-[7px] uppercase tracking-[0.18em] text-[#9a6f37]">
            Featured realized asset
          </p>
          <h2 className="mt-1 truncate text-[15px] font-semibold text-[#243d33]">
            {asset.project}
          </h2>
          <p className="mt-1 truncate text-[9px] text-[#747d78]">
            {getSoldDisplayAddress(asset)}
          </p>
        </div>
        <p className="shrink-0 text-[12px] font-semibold text-[#243d33]">
          {area.value > 0
            ? `${formatNumber(area.value)} ${area.suffix}`
            : "—"}
        </p>
          ...
          <div
            ...
            className={`grid min-h-[27px] grid-cols-[1fr_86px] items-center gap-3 px-3.5 py-1.5 text-[9px] ${
              index !== assets.length - 1
                ? "border-b border-[#e2ddd3]"
                : ""
            }`}
          >
            <p className="truncate font-medium text-[#52615b]">
              {getSoldDisplayAddress(asset)}
            </p>
            <p className="whitespace-nowrap text-right font-semibold text-[#243d33]">
              {area.value > 0

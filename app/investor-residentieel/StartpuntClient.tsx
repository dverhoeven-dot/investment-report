...
export default function StartpuntClient({
  ...
  async function saveToGoogleSheets() {
    ...
    setSheetMessage("Bezig met opslaan in Google Sheets…");

    try {
      const response = await fetch("/api/startpunt/save", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          input: {
            objectNaam: data.objectNaam,
            adres: form.adres,
            marktwaardeVastgoed: data.marktwaarde,
            wozWaardeVastgoed: data.wozwaarde,
            financiering: data.financiering,
            rentePercentage: data.rente,
            huurPerMaand: data.huurPerMaand,
            fiscaalPartner: form.fiscaalPartner,
            heffingsvrijBenut: form.heffingsvrijBenut,
          },
          exploitatiekosten: data.costRows.map((item) => ({
            name: item.name,
            amount: item.amount,
          })),
        }),
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Opslaan in Google Sheets is mislukt."
        );
      }
      ...
      const skippedText =
        result.skipped && result.skipped.length > 0
          ? ` Niet gevonden in de Sheet: ${result.skipped.join(", ")}.`
          : "";

      setSheetMessage(`Opgeslagen in Google Sheets.${skippedText}`);
    } catch (error) {
      setSheetSaveStatus("error");
      setSheetMessage(
        error instanceof Error
          ? error.message
          : "Opslaan in Google Sheets is mislukt."
      );
    }
  }

  function reloadFromGoogleSheets() {
    localStorage.removeItem(STORAGE_KEY);
      ...
      <section className="case-editor no-print">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            onClick={reloadFromGoogleSheets}
          >
            Laad uit Google Sheets
          </button>
          ...
          {sheetMessage && (
            <p
              className={`sheet-status ${
                sheetSaveStatus === "error" ? "error" : "success"
              }`}
              role="status"
            >
              {sheetMessage}
...
const styles = `
  ...
  * {
  ...
  .case-editor button.sheet-save-button {
    border-color: var(--taupe);
    background: var(--taupe);
    color: white;
  }

  .case-editor button.sheet-load-button {
    background: #fffdf8;
  }

  .case-editor button:disabled {
    cursor: wait;
    opacity: 0.65;
  }
